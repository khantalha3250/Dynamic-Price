...
def generate_synthetic_data(consolidated_prices, num_entries=100, random_seed=None):
    ...
    times_of_day = ["10 AM","11 AM","12 PM","1 PM","2 PM","3 PM","5 PM","6 PM","7 PM","8 PM","9 PM","10 PM"]
...
def save_to_csv(dataset, consolidated_prices, file_name="menu_pricing_dataset.csv"):

    with open(file_name, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)

        headers = ["temperature_f", "conditions", "rain_mm", "day", "time", "busyness_percentage"]
        headers.extend([f"{item}_price" for item in consolidated_prices.keys()])
        ...
        for entry in dataset:
            ...
            row.extend([entry["items_price"][item] for item in consolidated_prices.keys()])
            writer.writerow(row)
if __name__ == "__main__":
    ...
    file_name = f"menu_pricing_dataset{timestamp}.csv"
    save_to_csv(dataset, consolidated_prices, file_name=file_name)
